...
IF NOT EXIST "silent_update_MD.bat" (
  IF %colorSupport%==Yes (
    echo [31m⚠[0m  缺少 [32msilent_update_MD.bat[0m 檔案，請檢查資料表產生器檔案是否完整。現在即將使用預設瀏覽器帶您前往資料表產生器下載網頁，請按任意鍵繼續……
  ) ELSE (
    echo 錯誤：缺少 silent_update_MD.bat 檔案，請檢查資料表產生器檔案是否完整。現在即將使用預設瀏覽器帶您前往資料表產生器下載網頁，請按任意鍵繼續……
  )
  pause > nul
...
IF NOT EXIST "MD.xsl" (
  IF %colorSupport%==Yes (
    echo [31m⚠[0m  缺少 [32mMD.xsl[0m 檔案，無法處理資料表。請把 MD.xsl 跟 silent_update_MD.bat 放在同一個資料夾內，然後再試一次。請按任意鍵結束……
  ) ELSE (
    echo 錯誤：缺少 MD.xsl 檔案，無法處理資料表。請把 MD.xsl 跟 silent_update_MD.bat 放在同一個資料夾內，然後再試一次。請按任意鍵結束……
  )
  pause > nul
  exit
)
...
IF NOT EXIST "MD.html" (
  IF %colorSupport%==Yes (
    echo [36m🔰[0m 未發現 [32mMD.html[0m 檔案，無法確認是否已能正確處理資料表。現在即將執行 [33mupdate_MD.bat[0m，請在確認成功後再嘗試加入排程自動執行。
  ) ELSE (
    echo 未發現 MD.html 檔案，無法確認是否已能正確處理資料表。現在即將執行 update_MD.bat，請在確認成功後再嘗試加入排程自動執行。
  )
  echo.
  echo 請按任意鍵繼續……
  pause > nul
  IF NOT EXIST "update_MD.bat" (
    IF %colorSupport%==Yes (
      echo [31m⚠[0m  缺少 [32mupdate_MD.bat[0m 檔案，請檢查資料表產生器檔案是否完整。現在即將使用預設瀏覽器帶您前往資料表產生器下載網頁，請按任意鍵繼續……
    ) ELSE (
      echo 錯誤：缺少 update_MD.bat 檔案，請檢查資料表產生器檔案是否完整。現在即將使用預設瀏覽器帶您前往資料表產生器下載網頁，請按任意鍵繼續……
    )
    pause > nul
    start "" "https://github.com/JediLin/Taiwan-Assistive-Technology-medical-device-permit-license-data-table-generator/releases/latest"
    exit
...
echo.    在每季（一月、四月、七月、十月）的第二個星期三，下午兩點左右執行
echo.
IF %colorSupport%==Yes (
  echo [32m動作[0m
) ELSE (
...
echo.   cmd /C start "自動更新醫材字號資料表" /D "%cd%" "%~dp0silent_update_MD.bat" 
echo.
IF %colorSupport%==Yes (
...
:doupdate
echo.
schtasks /Create /SC MONTHLY /M JAN,APR,JUL,OCT /MO SECOND /D WED /ST 14:05 /TN "自動更新醫材字號資料表" /TR "cmd /C start '自動更新醫材字號資料表' /D '%cd%' '%~dp0silent_update_MD.bat'" /F
echo.
echo 設定完成。
echo.
...
echo.
IF %colorSupport%==Yes (
  echo [31m⚠[0m  警告：本操作將刪除任何名稱為「[32m自動更新醫材字號資料表[0m」的排程項目。
) ELSE (
  echo 警告：本操作將刪除任何名稱為「自動更新醫材字號資料表」的排程項目。
)
echo.
...
:doremove
echo.
schtasks /Delete /TN "自動更新醫材字號資料表" /F
echo.
echo 排程已刪除。
echo.
